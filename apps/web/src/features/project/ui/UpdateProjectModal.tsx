...
export default function UpdateProjectModal() {
  ...
  const { isOpen } = useModalState({ key: MODAL.UPDATE_PROJECT });
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const handleFormSubmit = async (data: UpdateProjectRequest) => {
    await updateProjectInfo({ ...data, projectId: Number(projectId) });
    window.location.reload();
    closeModal(MODAL.UPDATE_PROJECT);
  };
        ...
        <Modal.Body className="h-full justify-between">
          <form onSubmit={handleSubmit(handleFormSubmit)}>
            ...
            <Modal.Footer className="mt-[44px] flex justify-end">
              <Button type="submit" className="w-[100px]" color="normal">
                저장하기
              </Button>
            </Modal.Footer>
          </form>
